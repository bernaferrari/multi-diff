...
export function createSearchHighlightPostRender(searchTarget: SearchNavigationTarget | null) {
  return (node: HTMLElement, instance: unknown) => {
    applyContentSearchHighlight(
      node,
      getSearchHighlightTarget(searchTarget, getRenderedDiffFileName(instance)),
    );
  };
}
...
function applyContentSearchHighlight(root: HTMLElement, target: SearchHighlightTarget | null) {
...
function clearContentSearchHighlights(root: HTMLElement) {
...
function getTargetLines(root: HTMLElement, target: SearchHighlightTarget) {
...
function wrapTextNodeSegment(node: Text, start: number, end: number) {
  ...
  mark.style.backgroundColor = "rgb(250 204 21 / 0.36)";
  mark.style.borderRadius = "3px";
  mark.style.boxShadow = "0 0 0 1px rgb(250 204 21 / 0.24)";
