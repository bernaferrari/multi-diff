...
describe("buildWorkbenchViewModel", () => {
  const parsed = [
    testParsedPane("a", [
      ...
      testFileDiff("a-only.ts", 3, 0),
    ]),
    testParsedPane("b", [
      testFileDiff("shared.ts", 4, 2),
  ...
  it("removes hidden files from pane render plans and tracks restorable rows", () => {
    ...
    expect(model.paneViews.get("a")?.files.map((item) => item.name)).toEqual([
      "a-only.ts",
    ])
  ...
  it("falls back to overview when focused file is no longer visible", () => {
    ...
    expect(model.displayedPaneViews.map(({ pane }) => pane.id)).toEqual([
      "a",
      "b",
      "c",
    ])
  ...
  it("narrows displayed panes and pane views to a focused file", () => {
    ...
    expect(model.paneViews.get("b")?.files.map((item) => item.name)).toEqual([
      "b-only.ts",
    ])
  ...
  it("filters hidden files in overview but keeps a focused file visible", () => {
    ...
    const model = buildWorkbenchViewModel({
      ...
      parsed: [
        testParsedPane("a", [
          testFileDiff("visible.ts", 1, 0),
          testFileDiff(hiddenFocused, 0, 1),
        ]),
      ],
    })

    expect(model.focused).toBeNull()
    expect(model.paneViews.get("a")?.files.map((file) => file.name)).toEqual([
      "visible.ts",
    ])
    ...
    expect(
      focusedModel.paneViews.get("a")?.files.map((file) => file.name)
    ).toEqual([hiddenFocused])
  })
})
