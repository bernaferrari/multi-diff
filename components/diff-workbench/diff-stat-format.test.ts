...
describe("diff stat formatting", () => {
  it("formats compact visual stat text consistently", () => {
    ...
    expect(formatDiffStatSummary({ additions: 12, deletions: 3 })).toBe("+12 −3");
  });

  it("formats accessible stat text without symbols", () => {
    expect(formatDiffStatLabel({ additions: 12, deletions: 3 })).toBe("12 additions, 3 deletions");
    expect(formatDiffStatLabel({ additions: 0, deletions: 0 })).toBe("no changes");
  });
