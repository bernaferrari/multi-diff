...
export function getActiveRowsFile(scroller: RowViewportScroller) {
  const blocks = Array.from(
    scroller.querySelectorAll("[data-row-file-name]")
  )
  if (blocks.length === 0) return null

  const scrollerRect = scroller.getBoundingClientRect()
  const activationLine =
    scrollerRect.top + Math.min(96, Math.max(32, scrollerRect.height * 0.18))
  let active: { laneId: LaneId; name: string } | null = null

  for (const block of blocks) {
    const blockRect = block.getBoundingClientRect()
    if (
      blockRect.top <= activationLine &&
  ...
  return firstName && firstLaneId
    ? { laneId: firstLaneId as LaneId, name: firstName }
    : null
}
