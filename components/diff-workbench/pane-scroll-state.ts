...
type PaneScrollCancelableInstance = Pick<
  CodeViewInstance,
  "getScrollTop" | "scrollTo"
>
...
export function cancelProgrammaticScroll(
  instance: PaneScrollCancelableInstance | undefined
) {
  if (!instance) return
  instance.scrollTo({
    type: "position",
...
export function isScrollDriver(
  driver: PaneScrollDriver,
  sourceId: LaneId,
  now = Date.now()
) {
