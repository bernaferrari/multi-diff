...
describe("file tree row labels", () => {
  ...
  it("builds directory labels from expansion state", () => {
    ...
    ).toBe("Expand folder app/api")
    expect(
      getDirectoryTreeRowTitle({ collapsed: true, path: "app/api" })
    ).toBe("Expand app/api")

    expect(
      getDirectoryTreeRowLabel({ collapsed: false, path: "app/api" })
    ).toBe("Collapse folder app/api")
    expect(
      getDirectoryTreeRowTitle({ collapsed: false, path: "app/api" })
    ).toBe("Collapse app/api")
  })
})
