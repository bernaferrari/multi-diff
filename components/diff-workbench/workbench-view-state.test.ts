...
describe("workbench view state", () => {
  it("partitions file rows by hidden filename", () => {
    ...
    expect(
      partitionFileRows([visible, hidden], new Set(["hidden.ts"]))
    ).toEqual({
      fileRows: [visible],
  ...
  it("narrows displayed panes to panes containing the focused file", () => {
    ...
    expect(getDisplayedPanes(panes, "focused.ts").map((pane) => pane.id)).toEqual([
      "b",
    ])
  })
  ...
  it("counts files shared by every pane with visible files", () => {
    ...
    const visiblePanes = [
      testParsedPane("a", [
        testFileDiff("shared.ts", 1, 0),
        testFileDiff("a-only.ts", 1, 0),
      ]),
