...
export function getFirstVisibleFileTreeName(
  rows: FileRow[],
  collapsedPaths = new Set<string>()
) {
  return getFirstVisibleFileTreeRowName(
    buildVisibleFileTreeRows(rows, collapsedPaths)
  )
}
...
function getFirstVisibleFileNode(treeRows: VisibleFileTreeRow[]) {
  return treeRows.find((item) => item.node.kind === "file" && item.node.row)
    ?.node
}
