...
export function getFileVisibilityPatch(names: Iterable<string>) {
  ...
  return {
    clearSelection: (current: string | null) =>
      clearSelectionIfHidden(current, selection),
...
export function getHiddenFileNameState(
  fileNames: string[],
  hiddenFiles: Set<string>
) {
  const hiddenNames = fileNames.filter((name) => hiddenFiles.has(name))
  const allHidden =
    fileNames.length > 0 && hiddenNames.length === fileNames.length
