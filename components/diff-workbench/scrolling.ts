...
export function routeWheelToScroller(event: WheelLike, scroller: ScrollTarget) {
  ...
  if (preferHorizontal) {
    ...
    const nextLeft = clampScrollPosition(scroller.scrollLeft + horizontalDelta, maxLeft);
  ...
  const nextTop = clampScrollPosition(scroller.scrollTop + verticalDelta, maxTop);
...
function setScrollPosition(scroller: ScrollTarget, position: { left?: number; top?: number }) {
