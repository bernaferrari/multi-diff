...
import type { DiffStyle, LaneId, LaneMarkerStyle, Layout, Pane } from "../shared/types";
import { getWorkbenchPersistenceState } from "../persistence/persistence";
import { useWorkbenchPersistence } from "../persistence/use-workbench-persistence";
import { createInitialWorkbenchState, type WorkbenchSetters } from "./workbench-state-model";
...
export function useWorkbenchState() {
  ...
  const [laneMarkerStyle, setLaneMarkerStyle] = useState<LaneMarkerStyle>(initial.laneMarkerStyle);
  ...
  const [hiddenFiles, setHiddenFiles] = useState<Set<string>>(initial.hiddenFiles);
  ...
  const persistedState = useMemo(() => getWorkbenchPersistenceState(state), [state]);
