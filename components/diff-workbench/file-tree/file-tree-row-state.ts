...
import { getFileTreeRowLabel, getFileTreeRowTitle } from "./file-tree-row-labels";
import type { FileRow, LaneId } from "../shared/types";

const FILE_TREE_ROW_BASE_CLASS =
  "group relative mb-0.5 flex h-7 w-full items-center gap-1.5 rounded-md border pr-1.5 text-left text-[12px] transition-colors content-visibility-auto [contain-intrinsic-size:28px]";
...
function getActiveLaneBorderGradient(laneIds: LaneId[]) {
  ...
  const stops = laneIds.map((id) => `color-mix(in oklch, var(--lane-${id}) 68%, transparent)`);
