...
import { testFileDiff, testPaneView as buildTestPaneView } from "../shared/test-builders";
...
describe("scroll spy state", () => {
  it("selects the latest diff before the activation line", () => {
    ...
    expect(getActiveScrollFile(view, metrics({ height: 200, scrollTop: 110 }))).toEqual({
      intraOffset: 10,
      name: "b.ts",
      top: 100,
    });
  ...
  it("falls back to the first diff when no item top is before activation", () => {
    ...
    expect(getActiveScrollFile(view, metrics({ height: 100, scrollTop: 0 }))).toMatchObject({
      name: "a.ts",
      top: 0,
    });
  });
  ...
  it("returns null for empty views", () => {
    expect(
      getActiveScrollFile(testPaneView([]), metrics({ height: 100, scrollTop: 0 })),
    ).toBeNull();
  });
});
