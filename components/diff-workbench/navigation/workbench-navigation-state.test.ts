...
describe("workbench navigation state", () => {
  ...
  it("reduces explicit navigation into one internal state transition", () => {
    const next = reduceWorkbenchNavigationState(initialWorkbenchNavigationState, {
      displayedPaneViews: [
        paneLookup("a", ["first.ts"]),
        paneLookup("b", ["first.ts", "second.ts"]),
      ],
  ...
  it("can reduce lane-specific navigation without changing matching sibling lanes", () => {
    const next = reduceWorkbenchNavigationState(
      ...
      {
        displayedPaneViews: [
          paneLookup("a", ["route.ts"]),
          paneLookup("b", ["route.ts"]),
          paneLookup("c", ["route.ts", "audit.ts"]),
        ],
        laneIds: ["b"],
        name: "route.ts",
        navigationLockUntil: 1000 + NAVIGATION_SCROLL_SPY_LOCK_MS,
    ...
    expect(next.navigationTarget).toMatchObject({
      laneIds: ["b"],
      name: "route.ts",
    });
  });

  it("limits active file lookup to requested lanes when provided", () => {
    expect(
      getNavigationActiveFileByLane({
        currentActiveFileByLane: { c: "other.ts" },
        displayedPaneViews: [
          paneLookup("a", ["shared.ts"]),
          paneLookup("b", ["shared.ts"]),
          paneLookup("c", ["shared.ts", "other.ts"]),
        ],
        laneIds: ["a"],
        name: "shared.ts",
      }),
  ...
  it("locks scroll-spy updates for a short window after navigation", () => {
    ...
    expect(isNavigationScrollLocked({ lockUntil, now: lockUntil + 1 })).toBe(false);
  });
  ...
  it("uses the active, indexed, or first tree file as navigation fallback", () => {
    const fileRows = [fileRow("components/result-list.tsx"), fileRow("app/api/search/route.ts")];
  ...
  it("uses the first visible tree file as the initial focus target", () => {
    expect(
      getFocusModeNavigationTarget({
        ...
        fileRows: [fileRow("components/result-list.tsx"), fileRow("app/api/search/route.ts")],
        indexActiveFile: null,
      }),
    ).toBe("app/api/search/route.ts");
  });
