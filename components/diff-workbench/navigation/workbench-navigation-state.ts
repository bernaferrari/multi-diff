import type { ActiveFileByLane, FileNavigationTarget, FileRow, LaneId } from "../shared/types";

import { getFirstVisibleFileTreeName } from "../file-tree/file-tree";
...
export function getNavigationActiveFileByLane({
  ...
  return {
    ...seed,
    ...getActiveFileByVisibleLanes(
      laneIds
        ? displayedPaneViews.filter(({ pane }) => laneIds.includes(pane.id))
        : displayedPaneViews,
      name,
    ),
  };
